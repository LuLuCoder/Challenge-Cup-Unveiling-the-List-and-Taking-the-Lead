...
- **数据导入**：一次多选节点坐标文件与 X/Y/Z/XY/YZ/XZ 六个应力分量文件，
  按文件名与内容自动识别分类（与参数优化模块一致的导入方式）；也可直接
  导入 STEP(.step/.stp) 文件：软件调用 gmsh（OpenCASCADE 内核）自动划分
  四面体网格，以网格节点作为点云，新零件无需 ANSYS 导出 CSV；
- **STEP 网格检查窗口**：导入 STEP 后弹出四视图（模板形状 / STEP 线框 /
  四面体网格 / 网格节点点云）与网格质量报告（minSICN 平均/最差质量、
  差单元占比、实体体积、自动判定"优秀/良好/需细化"），可一键导出网格点云 CSV；
  主界面与模板映射窗口均提供「STEP 检查」按钮，可随时重新打开最近一次
  网格结果查看；
...
- **路径规划**：沿切片轴（默认 Z）分层，层内生成应力自适应间距的锯齿扫描线
  ...
  实体内低应力区域全部保留；
- **可视化**：主窗口与模板映射窗口均为四个独立 3D 画布（模型几何、应力分析、
  规划路径、虚拟机械臂），只有机械臂画布实时动画；**6 自由度关节臂**末端
  精确贴合路径、零件放大显示；
  双击任意图可弹出放大窗口（机械臂放大窗口为"左：机械臂动画 / 右：零件
  路径完成进度"双窗）；节点云全量显示，路径用批量绘制不糊图；
- **性能**：导入与路径生成在后台线程执行（主界面与模板映射窗口通用），
  界面不冻结；13.8 万节点导入约 2 秒、规划约 2 秒。
- **相似零件模板映射（免重复仿真）**：真实仿真 + 路径规划结果可存入模板库；
  新零件只导入节点坐标文件（或直接导入 STEP，自动网格化取点云），系统自动
  评判与模板的形状相似度（主轴坐标/径向直方图 L1 距离，阈值可调），相似度
  达标则映射模板应力场并在新零件上按层重规划路径（仍是层式 3D 打印结构），
  无需重新做 ANSYS 分析。
...
依赖：`numpy pandas matplotlib scipy gmsh`（测试：`pytest`）。
...
- ③ 相似零件模板映射标签页与路径规划共用四画布 + 后台线程架构，
  大量点云导入不卡；真实仿真数据导入后自动规划路径，便于直接建模板；
...
- **STEP 直接导入（替代 CSV 导出）**：新零件无需先做 ANSYS 网格导出，
  直接选 `.step/.stp` 文件，路径规划模块用 gmsh 自动划分四面体网格、
  以网格节点作为点云进行模板匹配与路径规划。
...
```bash
pip install numpy pandas matplotlib scipy scikit-learn gmsh ultralytics opencv-python
```
...
│   │   │                          # + STEP→gmsh 四面体网格→点云（step_mesh.py）
│   │   ├── analysis/             # 应力分析 + 层式规划 + 形状签名 + 模板库
│   │   ├── visualization/        # 绘图 + 6 自由度虚拟机械臂
│   │   └── ui/                   # tkinter 主窗口 + 模板映射窗口（共用四画布架构）
│   │                              # + STEP 四视图网格检查窗口（可随时回看）
│   └── tests/
├── satc_optimizer/               # ② 参数优化
│   ├── app.py                    # GUI 入口
│   ├── main.py                   # 命令行入口
