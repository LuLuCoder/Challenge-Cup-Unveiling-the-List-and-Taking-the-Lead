...
| 环节     | 模块            | 输入                        | 输出                                            | 关键技术                                  |
| -------- | --------------- | --------------------------- | ----------------------------------------------- | ----------------------------------------- |
| 打印前   | ① 路径规划     | ANSYS 节点坐标 + 六应力分量 | 层式 3D 打印路径                                | 应力自适应间距、空区检测、虚拟机械臂演示  |
...
- **路径规划**：沿切片轴（默认 Z）分层，层内生成应力自适应间距的锯齿扫描线
  ...
  实体内低应力区域全部保留；
- **可视化**：四个 3D 视图（模型几何、应力分析、规划路径、虚拟机械臂运动演示），
  路径溯源表格，支持保存融合数据与路径 CSV。
- **相似零件模板映射（免重复仿真）**：真实仿真 + 路径规划结果可存入模板库；
  新零件只导入节点坐标文件，系统自动评判与模板的形状相似度（主轴坐标/径向
  直方图 L1 距离，阈值可调），相似度达标则直接把模板路径与应力场映射到新零件，
  无需重新做 ANSYS 分析。
...
│   │   ├── analysis/             # 应力张量分析 + 层式路径规划
│   │   ├── visualization/        # 绘图 + 虚拟机械臂
│   │   └── ui/                   # tkinter 主窗口
│   └── tests/
├── satc_optimizer/               # ② 参数优化
│   ├── app.py                    # GUI 入口
